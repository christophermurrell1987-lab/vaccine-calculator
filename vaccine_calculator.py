...
# UPDATED: Specifically for Breeder/Parent Stock context
VACCINE_PROTECTION = {
...
# =============================================================================
# 🛡️ PROTECTION SUMMARY (UPDATED LAYOUT)
# =============================================================================
...
if diseases_covered:
    # Use 3 columns for better width and alignment
    ...
    for i, disease in enumerate(sorted_diseases):
        ...
        with prot_cols[col_index]:
            # APPLY CONSISTENT STYLING TO EVERYTHING
            # This ensures every item is a colored box, fixing the misalignment
            
            if "Marek's" in disease:
                st.success(f"🛡️ **{disease}**")
                ...
                # Default to Green (Success) for Reo, Bacterial etc so they are also boxes
...
with st.expander("📋 Click for Detailed Cost Breakdown"):
    ...
    # Logic to build detailed list
    # INOVO
    if inovo_1 != "-- Select --":
        cost = INOVO_VACCINES[inovo_1]["price_per_1000"] * inovo_multiplier_doses
        breakdown_data.append((f"Inovo: {inovo_1}", cost))
    if inovo_2 != "-- Select --":
        cost = INOVO_VACCINES[inovo_2]["price_per_1000"] * inovo_multiplier_doses
        breakdown_data.append((f"Inovo: {inovo_2}", cost))
    if inovo_3 != "-- Select --":
        cost = INOVO_VACCINES[inovo_3]["price_per_1000"] * inovo_multiplier_doses
        breakdown_data.append((f"Inovo: {inovo_3}", cost))
    if inovo_selected:
        breakdown_data.append(("Inovo Machine Lease", inovo_machine_cost))

    # DAY OLD INJ
    if day_old_inj_1 != "-- Select --":
        cost = DAY_OLD_INJECTION_VACCINES[day_old_inj_1]["price_per_1000"] * chick_multiplier_doses
        breakdown_data.append((f"Day Old Inj: {day_old_inj_1}", cost))
    if day_old_inj_2 != "-- Select --":
        cost = DAY_OLD_INJECTION_VACCINES[day_old_inj_2]["price_per_1000"] * chick_multiplier_doses
        breakdown_data.append((f"Day Old Inj: {day_old_inj_2}", cost))
    if day_old_inj_3 != "-- Select --":
        cost = DAY_OLD_INJECTION_VACCINES[day_old_inj_3]["price_per_1000"] * chick_multiplier_doses
        breakdown_data.append((f"Day Old Inj: {day_old_inj_3}", cost))
    if day_old_inj_selected:
        breakdown_data.append(("Novatech Machine Costs", novatech_machine_cost))
    
    # SPRAY
    if day_old_spray_1 != "-- Select --":
        cost = DAY_OLD_SPRAY_VACCINES[day_old_spray_1]["price_per_1000"] * chick_multiplier_doses
        breakdown_data.append((f"Spray: {day_old_spray_1}", cost))
    if day_old_spray_2 != "-- Select --":
        cost = DAY_OLD_SPRAY_VACCINES[day_old_spray_2]["price_per_1000"] * chick_multiplier_doses
        breakdown_data.append((f"Spray: {day_old_spray_2}", cost))
    if day_old_spray_3 != "-- Select --":
        cost = DAY_OLD_SPRAY_VACCINES[day_old_spray_3]["price_per_1000"] * chick_multiplier_doses
        breakdown_data.append((f"Spray: {day_old_spray_3}", cost))
